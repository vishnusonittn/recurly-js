...
class ApplePay extends Emitter {
  ...
  onPricingChange (price) {
    ...
    // Plan
    this.config.lineItems.push({ type: 'final', label: this.planLabel, amount: price.now.plan });

    // Add-ons are repreented as a single line item
    if (!!this.config.pricing.items.addons.length) {
      this.config.lineItems.push({ type: 'final', label: 'Add-ons', amount: price.now.addons });
    }

    // Setup fee
    if (!!parseFloat(price.now.setup_fee)) {
      this.config.lineItems.push({ type: 'final', label: 'Setup fee', amount: price.now.setup_fee });
    }

    // Discount
    if (!!parseFloat(price.now.discount)) {
      this.config.lineItems.push({ type: 'final', label: 'Discount', amount: `-${price.now.discount}` });
    }

    // Taxes
    if (!!parseFloat(price.now.tax)) {
      this.config.lineItems.push({ type: 'final', label: 'Taxes', amount: price.now.tax });
    }
